...
export default async function DevamEdenProjeler() {
    ...
    <main>
      <section className="section bg-light" style={{ paddingBottom: 0 }}>
        <div className="container">
          <h1 className="section-title">Devam Eden Projeler</h1>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container">
          <div className="grid-cards">
            {projects.length === 0 ? (
              <p style={{ textAlign: 'center', width: '100%', padding: '3rem' }}>Şu an için listelenecek devam eden proje bulunmamaktadır.</p>
            ) : (
              projects.map(project => (
                <Link key={project.id} href={`/projeler/${project.id}`} className="card">
                  ...
                  <div className="card-content">
                    ...
                    <p className="card-price">{project.startingPrice} TL'den başlayan fiyatlarla</p>
                    <div className="btn-primary" style={{ width: "100%", marginTop: 'auto' }}>Daireleri İncele</div>
                  </div>
                </Link>
              ))
            )}
